    ...
    var MERCADO_JS = {
		...
		mercado_toggle_slide_menu: function() {
			...
			if($(".widget .list-limited .default-hiden").length > 0){
				$(document).on( 'click', '.widget .list-limited .btnx-control', function(e){
    				e.preventDefault();
    				var _this = $(this),
			...
			if($(".toggle-slide-menu").length > 0){
				$(".toggle-slide-menu").on( 'click', '.btnx-control a', function(el){
					el.preventDefault();
					var _this = $(this);
		...
		mercado_price_quantity: function(){
			if($(".quantity-input").length > 0){
				$(".quantity-input").on('click', '.btnx', function(event) {
					event.preventDefault();
					var _this = $(this),
		...
		mercado_remove_product_in_cart: function(){
			if($(".products-cart .pr-cart-item").length > 0){
				$(document).on('click', '.pr-cart-item .delete .btnx-delete', function(event) {
					event.preventDefault();
